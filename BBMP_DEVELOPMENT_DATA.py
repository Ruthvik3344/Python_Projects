...
def fetch_complete_row_data(parent_record, ward_id):
    ...
    # 1. Extract correct start and end dates from the job code HTML
    ...
    if isinstance(approvals,list) and approvals:
        for item in approvals:
            rows.append({
                ...
                # 2. Use the extracted start and end dates here
                ...
                "Milestone_Date": clean_html(item.get("date", "")),
    ...
    return [{
        ...
        # 3. And also use them here for the fallback return block
        "Start_Date": start_date,
        "End_Date": end_date,
...
# =====================================================
# PROCESS ENTIRE WARD (BULLETPROOF VERSION)
# =====================================================
...
def process_ward(ward, financial_years):
    ...
    try:
        ...
        if not parent_records:
            print(f"   No parent records found for {ward_name}.")
            return None
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logging.error(f"Row fetch error in {ward_name}: {ex}")

                print(f"\r   {completed}/{len(parent_records)}", end="")

        print()

        # Safe sorting logic with fallback
        try:
            ...
            output_rows.sort(
                key=lambda row: (
                    -financial_year_key(row),
                    int(row.get("Main_Serial_No", 0))
                    if str(row.get("Main_Serial_No", "")).isdigit()
                    else 999999,
                    work_code_key(row)
                )
            )
        except Exception as sort_err:
            logging.warning(f"Sorting warning for {ward_name}: {sort_err}")
        # Assign continuous serial numbers uniquely per Work Bill ID
        serial_counter = 0
        seen_wbis = {}
        ...
        xlsx_name = safe_filename(ward["name"].replace(" ", "_")) + ".xlsx"
        xlsx_path = os.path.join(OUTPUT_FOLDER, xlsx_name)

        fieldnames = [
            "Ward_Query_ID/RID", "Financial_Year", "Start_Date", "End_Date",
            "Main_Serial_No", "Work_Code_Name", "Work_Bill_ID", "Name_of_Work",
            "Total_Amount", "Budget_Head", "Division", "Contractor_Name",
            "Mobile", "Bill_Type", "Gross_Amount", "Deductions", "Net_Amount",
            "Current_Level", "Detail_Row_ID", "Approval_Stage_Name",
            "Milestone_Date", "Approval_Remarks"
        ]
        ...
        currency_columns = ["Total_Amount", "Gross_Amount", "Deductions", "Net_Amount"]
        ...
        # Safe Merging Block
        merge_columns = [
            "Start_Date", "End_Date", "Main_Serial_No", "Work_Code_Name",
            "Work_Bill_ID", "Name_of_Work", "Total_Amount", "Budget_Head",
            "Division", "Contractor_Name", "Mobile", "Bill_Type",
            "Gross_Amount", "Deductions", "Net_Amount", "Current_Level"
        ]
        ...
        try:
            ...
            while start_row <= ws.max_row:
                ...
                end_row = start_row
                current_year = ws.cell(row=start_row, column=header_map.get("Financial_Year", 2)).value

                while (
                    end_row + 1 <= ws.max_row
                    and ws.cell(row=end_row + 1, column=header_map.get("Main_Serial_No", 1)).value == serial
                    and ws.cell(row=end_row + 1, column=header_map.get("Financial_Year", 2)).value == current_year
                ):
                    end_row += 1
            ...
            logging.warning(f"Merge warning for {ward_name}: {merge_err}")
        ...
        logging.exception(f"Critical error processing ward {ward_name}: {outer_ex}")
        print(f"   [ERROR] Failed completely on {ward_name}: {outer_ex}")
        return {
            "Ward": ward_name,
            "Parent": 0,
            "Rows": 0,
            "Status": f"Failed: {str(outer_ex)}"
        }
# =====================================================
# MAIN
# =====================================================
